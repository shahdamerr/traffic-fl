...
The primary aim of this thesis is to design, implement, and evaluate a novel \textbf{Hierarchical Federated Learning Framework} specifically optimized for communication-efficient and privacy-preserving traffic forecasting. 

To achieve this overarching aim, the thesis defines the following specific objectives:
\begin{enumerate}
    ...
    \item \textbf{Communication Efficiency and Robustness:} To drastically reduce uplink bandwidth requirements by integrating Top-K gradient sparsification \cite{gao2024fedce}, and to handle dynamic network conditions through an adaptive, reliability-based client selection mechanism coupled with quality-weighted aggregation \cite{dai2024}.
...
\begin{itemize}
    ...
    \item \textbf{Chapter 3: Methodology.} This chapter details the proposed Hierarchical Federated Learning framework. It provides an in-depth mathematical and architectural description of the DTW-based clustering algorithm, the Seq2Seq GRU local models, the Top-K gradient sparsification mechanism, the adaptive client selection policy, and the two-tier distance-weighted aggregation strategy.
    
    \item \textbf{Chapter 4: Ablation Study.} This chapter isolates the individual components of the proposed pipeline. It systematically evaluates the impact of clustering versus a flat global network, the efficacy of teacher forcing during local training, and the trade-offs involved in tuning the hierarchical mixing coefficient ($\alpha$) and Top-K compression rates.
